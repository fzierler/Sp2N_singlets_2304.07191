...
scatter!(plt1,[0],[0],markershape=:circle,mc=:white,label=L"\rho")
scatter!(plt1,[0],[0],markershape=:circle,mc=:grey,label=L"\eta'")
# plot data
for (n,i) in enumerate(collaboration_indices)
    ...
    scatter!(plt1,r[i],xerr=Δr[i],mρπ[i],yerr=Δmηπ[i],color=colors[1],ms=mss[n],marker=markers[n],label="")
    scatter!(plt2,r[i],xerr=Δr[i],mηρ[i],yerr=Δmηρ[i],color=colors[2],ms=mss[n],marker=markers[n],label="")
end
# now create legends
for (n,i) in enumerate(collaboration_indices)
    scatter!(plt1,[0],[0],color=:black,ms=mss[n],marker=markers[n],label=first(collab[i]))
    scatter!(plt2,[0],[0],color=:grey, ms=mss[n],marker=markers[n],label=first(collab[i]))
end
plot!(plt1,xlims=(0.12,0.9),ylims=(0.8,7))
plot!(plt2,xlims=(0.12,0.9),ylims=(0.55,1.35))

# plots for comparing gauge groups
plt3 = scatter(r,mηρ,yerr=Δmηρ,xerr=Δr,title="",legend=:outerright,color=:grey,label=L"\rm{SU(3)}",ms=8,markershape=:diamond)
plt4 = scatter(r,mηπ,yerr=Δmηπ,xerr=Δr,title="",legend=:topright,color=:grey,label=L"\rm{SU(3)}",ms=8,markershape=:diamond)
...
scatter!(plt3,x[iSU2],mηρ[iSU2],yerr=Δmηρ[iSU2],xerr=Δx[iSU2],color=:black,label=L"\rm{SU(2)}(\beta=2.0)",ms=8,markershape=:pentagon)
